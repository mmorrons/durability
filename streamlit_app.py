# streamlit_app.py (Multi-Section Version - Full Updated Code v5 - Custom Title / Zoom Fix)
...
try:
    # Ensure data_processing.py (Caching Disabled - Sec MA Index Approach Refined version) is in the same directory
...
# --- Configuration ---
APP_TITLE = "Multi-Analysis Segmenter (v5)"
...
def create_plot(df_display, x_col, y_col, smoothing_method, metadata, segments_list, section_id):
    ...
    # Check if columns exist before checking for NaNs
    ...
    fig.update_layout(
        ...
        dragmode='zoom' # Keep zoom default drag mode
    )
    return fig
...
for section in st.session_state.analysis_sections:
    ...
    surname = metadata.get('surname', 'UnknownSubject')
    y_var = config.get('y_col', 'Y')
    x_var = config.get('x_col', 'X')
    smooth_type = config.get('smooth', 'Raw Data')
    subject_name = f"{name} {surname}".strip()
    # Use display_name from metadata as a better fallback if name/surname missing
    ...
    with st.expander(expander_title, expanded=True):
            ...
            if uploaded_file is not None:
                ...
                if current_file_name != uploaded_file.name:
                    ...
                    try:
                        ...
                        metadata_parsed, unique_key = dp.parse_filename(filename)
                        ...
                        if raw_df is not None:
                            ...
                            if prepared_df is not None and not prepared_df.empty:
                                ...
                                section['metadata'] = metadata_parsed or {'filename': filename, 'display_name': filename} # Store parsed metadata
                                st.success(f"Loaded: {filename}")
                                st.rerun() # Rerun needed to update title, config options etc.
                        ...
                        # Store basic file info even if processing failed
                        section['file_info'] = {'name': uploaded_file.name, 'unique_key': uploaded_file.name}


            # B. Configuration (Only show if data is successfully loaded)
            if section.get('prepared_df') is not None:
                st.markdown("---")
                df_prepared_sec = section['prepared_df']
                config = section['plot_config'] # Use direct reference
                needs_config_rerun = False # Flag to rerun if config causes title change
                ...
                if new_smooth != config['smooth']:
                     ...
                     section['plot_fig'] = None # Force plot recalc
                     needs_config_rerun = True

                # Get numeric cols (based on current smoothing selection for safety)
                numeric_cols = []
                try:
                    # We need df_display for cols, calculate it if needed or use fallback
                    if section.get('df_display') is None or needs_config_rerun: # Recalc if smoothing changed or df missing
                         section['df_display'] = dp.apply_smoothing(df_prepared_sec, config['smooth'], dp.TIME_COL_SECONDS)

                    df_display_check = section.get('df_display') # Get potentially updated df_display
                    if df_display_check is not None and not df_display_check.empty:
                         numeric_cols = df_display_check.select_dtypes(include=np.number).columns.tolist()

                    # Fallback if still no columns
                    if not numeric_cols:
                         logger.warning(f"No numeric cols after smoothing (Sect {section_id}). Falling back to prepared.")
                         numeric_cols = df_prepared_sec.select_dtypes(include=np.number).columns.tolist()
                except Exception as e_smooth_cols:
                    st.error(f"Error getting columns after smoothing: {e_smooth_cols}")
                    logger.error(f"Col selection smooth fail (Sect {section_id})", exc_info=True)
                ...
                # Rerun if any config change occurred to update title / force plot regen
                if needs_config_rerun:
                    # Update df_display one last time if needed before rerun
                    if section.get('df_display') is None or section.get('plot_fig') is None:
                        try:
                            section['df_display'] = dp.apply_smoothing(df_prepared_sec, config['smooth'], dp.TIME_COL_SECONDS)
                        except Exception as e_smooth_rerun:
                             st.error(f"Error applying smoothing before rerun: {e_smooth_rerun}")
                             section['df_display'] = None # Ensure plot shows error if smoothing fails
                    st.rerun()
        ...
        with col2:
            st.subheader("2. Plot")
            # Get data/config needed for plotting from section state
            ...
            metadata_sec = section.get('metadata', {})

            # Check if ready to plot
            if df_display_sec is not None and x_col_sec and y_col_sec:
                # Plot figure regeneration should be handled when config changes or segments added/reset
                # If plot_fig is None, create it.
                if section.get('plot_fig') is None:
                     logger.info(f"Regenerating plot for section {section_id}")
                     section['plot_fig'] = create_plot(
                         df_display_sec, x_col_sec, y_col_sec,
                         config_sec.get('smooth','Raw Data'),
                         metadata_sec, segments_sec, section_id
                     )
                ...
                else: st.warning("Plot could not be generated or is pending update.")
